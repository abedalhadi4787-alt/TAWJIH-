...
export default function Tawjih() {
  ...
  return (
    <div style={{ direction:"rtl", fontFamily:"'Cairo',sans-serif", background:C.cream, minHeight:"100vh", color:C.text }}>
      ...
      {tab === "about" && (
        <div style={{ maxWidth:700, margin:"0 auto", padding:"48px 20px", textAlign:"center" }}>
          ...
          <p style={{ fontSize:15, lineHeight:2, color:C.textSoft, marginBottom:28 }}>
            <strong style={{ color:C.red }}>توجيه</strong> هي منصة جزائرية متخصصة في التوظيف وفرص العمل، تربط أصحاب العمل بالباحثين عن عمل في جميع ولايات الجزائر. هدفنا بناء سوق عمل رقمي شفاف وفعّال.
          </p>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            {[["2,400+","وظيفة نشطة"],["850+","شركة موثوقة"],["18,000+","مسجّل"],["48 ولاية","تغطية وطنية"]].map(([v,l]) => (
              <div key={l} style={{ background:"#fff", borderRadius:14, padding:20, border:`1px solid ${C.grayBorder}`, boxShadow:"0 2px 10px rgba(212,23,43,0.06)" }}>
                <div style={{ fontSize:28, fontWeight:900, color:C.red }}>{v}</div>
                <div style={{ fontSize:13, color:C.gray, fontWeight:600, marginTop:4 }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ═══ MODAL ═══ */}
      {job && (
        <div onClick={() => setJob(null)} style={{ position:"fixed", inset:0, background:"rgba(40,0,5,0.72)", backdropFilter:"blur(5px)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:20, width:"100%", maxWidth:600, maxHeight:"88vh", overflowY:"auto", direction:"rtl" }}>
            <div style={{ background:`linear-gradient(145deg,${C.redDark},${C.red})`, padding:24, borderRadius:"20px 20px 0 0" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
                <div style={{ display:"flex", gap:14, alignItems:"center" }}>
                  <div style={{ width:52, height:52, borderRadius:14, background:"rgba(255,255,255,0.2)", border:"2px solid rgba(255,255,255,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, fontWeight:900, color:"#fff" }}>{job.logo}</div>
                  <div>
                    <div style={{ color:"rgba(255,255,255,0.65)", fontSize:12, marginBottom:2 }}>{job.company}</div>
                    <div style={{ color:"#fff", fontSize:20, fontWeight:900 }}>{job.title}</div>
                  </div>
                </div>
                <button onClick={() => setJob(null)} style={{ background:"rgba(255,255,255,0.2)", border:"none", color:"#fff", width:34, height:34, borderRadius:8, cursor:"pointer", fontSize:16, display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
              </div>
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                {job.urgent && <span style={{ background:"rgba(255,255,255,0.2)", color:"#fff", fontSize:11, fontWeight:800, padding:"4px 12px", borderRadius:100 }}>⚡ توظيف عاجل</span>}
                {job.remote && <span style={{ background:"rgba(255,255,255,0.2)", color:"#fff", fontSize:11, fontWeight:800, padding:"4px 12px", borderRadius:100 }}>🌐 عمل عن بُعد</span>}
                <span style={{ color:"rgba(255,255,255,0.55)", fontSize:12, alignSelf:"center" }}>🕐 {job.posted}</span>
              </div>
            </div>

            <div style={{ padding:24 }}>
              <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:14 }}>
                {[["📍",job.location],["💼",job.type],["💰",job.salary]].map(([ic,v]) => (
                  <div key={v} style={{ background:"#FFF0F1", borderRadius:8, padding:"8px 14px", fontSize:13, fontWeight:700, color:C.red, display:"flex", gap:6 }}>{ic} {v}</div>
                ))}
              </div>

              <div style={{ marginBottom:18 }}>
                <div style={{ fontSize:14, fontWeight:800, marginBottom:10, display:"flex", gap:8, alignItems:"center" }}><span style={{ color:C.red }}>◈</span>وصف الوظيفة</div>
                <p style={{ fontSize:13, lineHeight:2, color:C.textSoft }}>{job.desc}</p>
              </div>

              <div style={{ marginBottom:18 }}>
                <div style={{ fontSize:14, fontWeight:800, marginBottom:10, display:"flex", gap:8, alignItems:"center" }}><span style={{ color:C.red }}>◈</span>المتطلبات</div>
                {job.requirements.map((r,i) => <div key={i} style={{ display:"flex", gap:8, marginBottom:6, fontSize:13, color:C.textSoft }}><span style={{ color:C.red }}>✓</span>{r}</div>)}
              </div>

              <div style={{ marginBottom:20 }}>
                <div style={{ fontSize:14, fontWeight:800, marginBottom:10, display:"flex", gap:8, alignItems:"center" }}><span style={{ color:C.red }}>★</span>المزايا</div>
                {job.benefits.map((b,i) => <div key={i} style={{ display:"flex", gap:8, marginBottom:6, fontSize:13, color:C.textSoft }}><span style={{ color:C.red }}>★</span>{b}</div>)}
              </div>

              <button style={{ width:"100%", background:`linear-gradient(135deg,${C.redDark},${C.redLight})`, color:"#fff", border:"none", borderRadius:12, padding:14, fontSize:15, fontWeight:900, cursor:"pointer", marginBottom:10 }}>📨 تقدم للوظيفة الآن</button>
              <button onClick={() => toggleSave(job.id)} style={{ width:"100%", background: saved.includes(job.id) ? "#FFF0F1" : "transparent", color:C.red, border:`2px solid ${C.red}`, borderRadius:12, padding:12, fontSize:14, fontWeight:700, cursor:"pointer" }}>
                {saved.includes(job.id) ? "★ تم الحفظ" : "☆ حفظ الوظيفة"}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* FOOTER */}
      <footer style={{ background:`linear-gradient(135deg,${C.redDark},${C.red})`, color:"rgba(255,255,255,0.75)", textAlign:"center", padding:"24px", marginTop:48, fontSize:13 }}>
        <div style={{ fontWeight:900, color:"#fff", fontSize:16, marginBottom:6 }}>توجيه · TAWJIH-DZ.COM</div>
        <div>جميع الحقوق محفوظة © 2025 · منصة التوظيف الجزائرية</div>
      </footer>
    </div>
  );
}
